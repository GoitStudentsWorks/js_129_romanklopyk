import{a as $,S as v,b as E,c as S,N as M,P as x,A as k}from"./assets/vendor-CHMthEz3.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))r(t);new MutationObserver(t=>{for(const o of t)if(o.type==="childList")for(const n of o.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&r(n)}).observe(document,{childList:!0,subtree:!0});function s(t){const o={};return t.integrity&&(o.integrity=t.integrity),t.referrerPolicy&&(o.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?o.credentials="include":t.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function r(t){if(t.ep)return;t.ep=!0;const o=s(t);fetch(t.href,o)}})();$.defaults.baseURL="https://deserts-store.b.goit.study/api";async function y(a,e={}){try{return(await $(a,e)).data}catch(s){console.log(s)}}console.log("utils.js");console.log("bestsellers");async function q(a){const e=new v({lines:12,length:6,width:4,radius:8,color:"#e39622"}),s=E.create(`
    ...
    `,{closable:!1,className:"dessert-modal-wrapper",onClose:()=>{document.body.classList.remove("no-scroll")}});document.body.classList.add("no-scroll"),s.show();const r=s.element().querySelector(".dessert-loader");e.spin(r);try{const{name:t,description:o,composition:n,price:u,rate:p,image:c}=await y(`/desserts/${a}`,{method:"GET"});s.element().innerHTML=`
            ...
            </div>`,s.element().querySelector(".close-modal-btn").addEventListener("click",()=>{s.close()});const w=document.querySelector(".dessert-modal-wrapper");w.addEventListener("click",h=>{h.target===w&&s.close()}),document.addEventListener("keydown",h=>{h.key==="Escape"&&s.close()})}catch(t){console.log(t),s.close()}finally{e.stop()}}function P(a){const e=Array.from({length:5},()=>`
    <div class="star">
  ...
  `}const d=document.querySelector(".category-list"),b=document.querySelector(".category-select"),m=document.querySelector(".desserts-list");document.querySelector(".loader");const i=document.querySelector(".load-more-btn");async function j(){const a=document.querySelector("#categories-loader"),e=document.querySelector(".category-wrapper"),s=new v({lines:12,length:6,width:4,radius:8,color:"#d18a1f"});try{s.spin(a);const r=await y("/categories",{method:"GET"}),t=r.map(n=>`
    ...
    `).join("");m.insertAdjacentHTML("beforeend",u);const p=m.children.length;console.log({loadedElements:p}),p>=o?(i.disabled=!0,i.textContent="Більше немає десертів",i.classList.remove("hidden"),i.classList.add("disabled-btn")):(i.disabled=!1,i.textContent="Завантажити ще",i.classList.remove("hidden","disabled-btn"))}catch(t){console.log(t)}finally{s.stop(),r.classList.add("hidden")}}H(m);function H(a){a.addEventListener("click",e=>{if(e.target.closest(".dessert-details-btn")){const s=e.target.closest(".dessert-details-btn").dataset.id;q(s)}})}d.addEventListener("click",a=>{if(a.target.closest(".category-item")){l=a.target.closest(".category-item").dataset.categoryId,g=1,i.disabled=!1,i.textContent="Завантажити ще";const e=d.children;Array.from(e).forEach(s=>{s.querySelector("button").classList.remove("active")}),a.target.closest(".category-item > button").classList.add("active"),b.value=l,f(l)}});b.addEventListener("change",a=>{i.disabled=!1,i.textContent="Завантажити ще",l=a.target.value,g=1;const e=d.children;Array.from(e).forEach(s=>{s.querySelector("button").classList.remove("active")}),d.querySelector(`[data-category-id="${l}"] > button`).classList.add("active"),console.log(l),f(l)});i.addEventListener("click",()=>{i.disabled||(g+=1,f(l,!0))});f(l);console.log("modal");console.log("about.js");console.log("feedback.js");const L=document.getElementById("reviews-container"),C=document.getElementById("loader"),N=document.getElementById("toast-container");function T(a){const e=document.createElement("div");e.className="toast",e.textContent=a,N.appendChild(e),setTimeout(()=>e.remove(),4e3)}async function B(){C.classList.remove("hidden");try{const a=await fetch("https://deserts-store.b.goit.study/api/feedbacks");if(!a.ok)throw new Error("Помилка сервера при отриманні відгуків");const r=((await a.json()).feedbacks||[]).slice(0,10);D(r),O()}catch{T("Не вдалося завантажити дані. Повторіть спробу пізніше.")}finally{C.classList.add("hidden")}}function D(a){L.innerHTML="",a.forEach(e=>{const s=e.rate||5,t=(Math.round(s*2)/2).toFixed(1).replace(".",""),o=document.createElement("div");o.className="swiper-slide",o.innerHTML=`
